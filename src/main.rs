...
fn part_2_main() {
    // Parse input into one long vector
    ...
    // Form multidimensional grid 140x140
    let array = parse_string_to_array(&content_as_line);
    let multi_array = form_multidimensional_array(&array, (140, 140));

    // Form into 2x2 windows in order to parse out combinations
    let window_array = form_window_array(&multi_array);

    // Form windows into boolean masks in order to parse dots easier where everything is T or F
    let boolean_mask = form_boolean_mask(&window_array);

    // Chunk to allow knowing what row you are in the grid
    let chunked_window_array = chunk_window_array(&boolean_mask, 139);

    // Find all True True pairings reflecting "non dots" (number or symbol)
    let coordinate_pairings: HashSet<((usize, usize), (usize, usize))> =
        extract_coordinates_for_all_non_dot_pairings(&chunked_window_array);

    // Filter down to keep only (number, *) or (*, number)
    // Helping to remove instances of (number, number), or ($, number) etc.
    ...
    // Establish a system where you index traced numbers against the coordinates of the * partner e.g (*, number)
    // This allows you to link numbers which share a * together
    // You must trace if you've already seen a number before due to iterating through as a 2x2 grid and finding
    // multiple elements of the same number if the * is found in the middle of a number, above it
    ...
    for store in coordinates_and_value_map.values() {
        if store.len() > 1 {
            let store_product: i32 = store
                .iter()
                .map(|traced_number| traced_number.parse::<i32>().unwrap())
                .product();
...
fn part_1_main() {
    ...
    // Filter down to keep only (number, symbol) or (symbol, number)
    let filtered_pairing: Vec<_> = coordinate_pairings
        .iter()
        .filter(|pairing| {
            let non_boolean_values =
    ...
    // Establish a system where you index traced numbers against the coordinates of itself
    // This allows you compensate for overlapping 2x2 windows in your grid iteration
