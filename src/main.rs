...
fn main() {
    ...
    let skinny_window_array = form_skinny_window_array(&multi_array);
    let boolean_mask = form_boolean_mask(&skinny_window_array);
    let chunked_window_array = chunk_window_array(&boolean_mask, 139);

    println!("{:?}", skinny_window_array.get(skinny_window_array.len()-2));
    ...
    for filtered_trio in filtered_trios.iter() {
        ...
        let traced_number_a = trace_whole_number_from_array_following_coordinates(&multi_array, &target_number.0);
        let traced_number_c = trace_whole_number_from_array_following_coordinates(&multi_array, &target_number.2);

        // dbg!(&traced_number_a, &traced_number_c);

        let traced_coordinates_total: i64 = traced_number_a.1.parse::<i64>().unwrap() + traced_number_c.1.parse::<i64>().unwrap();
        let traced_values_product: i64 = traced_number_a.0.parse::<i64>().unwrap() * traced_number_c.0.parse::<i64>().unwrap();
    ...
    let result: i64 = coordinates_and_value_map
        .values()
        .into_iter()
        .sum();
...
pub fn form_skinny_window_array(input: &ArrayBase<OwnedRepr<String>, Dim<[Ix; 2]>>,
) -> Vec<ArrayView<String, Dim<[Ix; 2]>>> {
    let windows: Vec<ArrayView<String, Dim<[Ix; 2]>>> =
        input.windows(Dim((3, 2))).into_iter().collect();
    windows
}
...
pub fn extract_coordinates_for_all_non_dot_trios(input: &Chunks<Array<bool, Dim<[Ix; 2]>>>,
) -> HashSet<((usize, usize), (usize, usize), (usize, usize))>{

    let mut non_dot_trio_coordinates: HashSet<((usize, usize), (usize, usize), (usize, usize))> = HashSet::new();
    ...
    for (anchor_row, chunk) in input.clone().enumerate() {
        ...
        for (chunk_column, window_element) in chunk.iter().enumerate() {
            ...
            // Build out tuples reflecting every possible combo within a 3x2 grid
            let combinations: [(bool, bool, bool); 4] =
                [(a1, b1, c1), (a2, b2, c2), (a1, b2, c1), (a2, b1, c2)];
            ...
            for (position, &combo) in combinations.iter().enumerate() {
                if combo == (true, true, true) {
                    ...
                    non_dot_trio_coordinates.insert((original_position_a, original_position_b, original_position_c));
                }
            }
        }
    }
...
mod tests {
    ...
    fn test_window_array() {
        ...
        let array = parse_string_to_array(giant_string);
        let multi_array = form_multidimensional_array(&array, (7, 2));
        let window_array = form_skinny_window_array(&multi_array);
    ...
    fn test_skinny_window_array() {
        ...
        let expected = Array2::from_shape_vec((3, 2), vec!["4", "6", ".", ".", ".", "."]).unwrap();
        assert_eq!(expected, window_array.get(0).unwrap())
    }
    ...
    fn test_extract_non_dot_trios_from_array() {
        ...
        let expected: HashSet<((usize, usize), (usize, usize), (usize, usize))> = HashSet::from([
            ((0, 2), (1, 3), (2, 2)),
            ((7, 6), (8, 5), (9, 6))
        ]);
    ...
    fn test_extract_numbers_adjacent_to_symbols_based_on_part_2_test_case() {
        ...
        for filtered_trio in filtered_trios.iter() {
            ...
            let traced_coordinates_total: i32 = &traced_number_a.1.parse::<i32>().unwrap() + &traced_number_c.1.parse::<i32>().unwrap();
            let traced_values_product: i32 = &traced_number_a.0.parse::<i32>().unwrap() * &traced_number_c.0.parse::<i32>().unwrap();
        ...
        let established_part_2_value_pairs_adjacent_to_star: i32 = coordinates_and_value_map
            .values()
            .into_iter()
            .sum();
