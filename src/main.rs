...
#[cfg(test)]
mod tests {
    use itertools::{interleave, peek_nth, Itertools, izip};
    ...
    fn test_masking_logic_to_find_all_that_are_not_a_dot() {
        ...
        let a1: bool = not_a_dot_mask.get(2).unwrap().get((0,0)).unwrap().to_owned();
        let a2: bool = not_a_dot_mask.get(2).unwrap().get((0,1)).unwrap().to_owned();
        let b1: bool = not_a_dot_mask.get(2).unwrap().get((1,0)).unwrap().to_owned();
        let b2: bool = not_a_dot_mask.get(2).unwrap().get((1,1)).unwrap().to_owned();
    ...
    fn test_chunking_windows_to_be_able_to_trace_mask_array_back_to_root_array(){
        ...
        // test against an expected window (used in previous tests)
        ...
        // todo make use of enumerate to translate back to main grid
